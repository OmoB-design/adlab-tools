...
function RoasContent() {
  ...
  const handleNext = () => {
    const roasNum = parseFloat(roas.replace(/[^0-9.]/g, '')) || 0
    router.push(`/platform?adSpend=${adSpend}&roas=${roasNum}`)
  }
        ...
        <div className="flex flex-1 flex-col gap-(--space-50) overflow-y-auto px-(--space-120) pt-(--space-120) pb-(--space-64)">
          ...
          >
            ...
            <div className="relative flex w-full items-center gap-(--space-10) overflow-hidden rounded-(--radius-3xl) border border-(--color-surface-stroke) bg-(--color-surface-primary) px-[15px] py-(--space-12) shadow-(--shadow-float)">
              ...
              <p className="text-caption-2 font-medium leading-tight text-(--color-text-heading-06)">
                ...
                <div key={i} className="flex flex-1 flex-col gap-(--space-10)">
                  <div
                    className={`h-[5px] w-full rounded-[4px] ${
                      i <= 1 ? 'bg-(--color-purple-400)' : 'bg-(--color-surface-fg-01)'
                    }`}
                  />
