  ...
  return (
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <main className="relative flex flex-1 flex-col overflow-hidden rounded-bl-(--radius-6xl) rounded-tl-(--radius-6xl) border border-(--color-surface-stroke) bg-(--color-surface-primary) shadow-(--shadow-panel)">
        ...
        <div className="flex flex-1 flex-col gap-(--space-50) overflow-y-auto px-(--space-120) pt-(--space-120) pb-(--space-64)">
          ...
          <div className="flex flex-col gap-(--space-64)">
                ...
                <div key={i} className="flex flex-1 flex-col gap-(--space-10)">
                  <div
                    className={`h-[5px] w-full rounded-[4px] ${
                      i <= 1 ? 'bg-(--color-blue-400)' : 'bg-(--color-surface-fg-01)'
                    }`}
                  />
                  <div className="flex gap-(--space-10) text-caption-2 font-medium leading-tight">
                    <span className="text-(--color-blue-400)">{ps.num}</span>
            ...
            <div className="flex flex-col gap-(--space-40)">
              ...
              <div className="flex flex-col" style={{ gap: '56px' }}>
                ...
                {/* Toggle + input — delay 0.45s */}
                <div
                  className="animate-fade-in-up flex flex-col gap-(--space-24)"
                  style={{ animationDelay: '0.45s' }}
                >
                  {/* Segmented toggle */}
                  <div className="flex h-[54px] w-full items-center gap-0 overflow-hidden rounded-[14px] border border-(--color-surface-stroke) bg-(--color-surface-primary) p-(--space-10) shadow-(--shadow-surface)">
                    <button
                      type="button"
                  ...
                  {/* ROAS input */}
                  <div className="flex w-full items-center gap-(--space-8) rounded-(--radius-xl) border border-(--color-surface-stroke) bg-(--color-surface-primary) px-[14px] py-(--space-12) shadow-(--shadow-input) transition-[border-color,box-shadow] duration-150 ease-in-out focus-within:border-(--color-blue-500) focus-within:shadow-[0px_0px_0px_2px_rgba(122,168,255,0.15)]">
